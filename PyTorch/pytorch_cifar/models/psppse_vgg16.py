...
class PSPPSELayer(nn.Module):
    def __init__(self, in_channel, channel, reduction=16):
        ...
        self.fc = nn.Sequential(
            nn.Linear(in_channel*21, in_channel*21 // reduction, bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(in_channel*21 // reduction, channel, bias=False),
            nn.Sigmoid()
        )
...
class VGGBlock(nn.Module):
    def __init__(self,in_channels, channels, stride=1):
        ...
        self.se = PSPPSELayer(in_channel=in_channels,channel=channels)
...
class PSPPSEVGG16(nn.Module):
    def __init__(self, num_classes=100,init_weights=True):
        super(PSPPSEVGG16, self).__init__()
...
def test():
    net = PSPPSEVGG16(num_classes=10)
